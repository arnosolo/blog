...
- 服务端 `Firebase` 或者 `Mongodb` + `Express` + `TS` + `Docker`
...
[Unblur Image](https://unblur-image.com/)使用人工智能技术将你的老照片变得清晰. 小图看得不是很清楚, 建议跳转到[网站主页](https://unblur-image.com/)查看大图.

#### Web端

- 使用 `Vite` + `Vue3` + `TS` + `Tailwind css` 构建, 适配各种屏幕尺寸

- 使用 `Github Action` 自动部署到aws
...
- 提交代码前(`git commit`)会自动进行语法检查(`eslint`), 并自动测试所有接口(`jest`)
